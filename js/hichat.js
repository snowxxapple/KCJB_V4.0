...
window.onload = function() {
    ...
    var oBottom = document.getElementById('historyMsg1');
    var oLine = document.getElementById('line');
    scrollScreen(document.getElementById('historyMsg'));
    scrollScreen(document.getElementById('historyMsg1'));
    scrollScreen(document.getElementById('home'));
    //聊天窗口 分栏线的拖拽
    oLine.onmousedown = function(e) {
        var disX = (e || event).clientX;
        var disY = (e || event).clientY;
    ...
    //         if (data.succ !== 0) {//succ代表服务器里有我没，非零代表没我
    //             $.ajax({
    //                     url: '/api/login',
    //                     type: 'POST',//告诉服务器我是游客，服务器会给我一个游客身份
    //                     dataType: 'json',
    //                 })
    //                 .done(function(msg) {
    //                     console.log("success", msg);
    //                     if (msg.succ == 0) {//服务器里有我了
    //                         networkRun();
    //                     }
    //                 });
    //         } else {
...
function networkRun() {
    $.ajax({
        ...
        success: function(data) {
            ...
            //根据用户的组别来添加对应的功能按钮 判断要重写
            if (__info.group == 0) {//组别
...
//左下角行情提醒 leftmention.js  要有查看更多功能
            $.ajax({
